...
//    public void mouseReleased(MouseEvent e){}
    ...
    MyFrame(String name) {
        ...
        field1.setBounds(label1.getX() + 80, label1.getY(), 150, 17);

        field2 = new JTextField();
        field2.setBounds(label2.getX() + 80, label2.getY(), 150, 17);

        field3 = new JTextField();
        field3.setBounds(label3.getX() + 80, label3.getY(), 150, 17);

        firstRequire = new JLabel("Choose type of series: ");
        firstRequire.setBounds(label3.getX(), label3.getY() + 30, 130, 17);

        secondRequire = new JLabel("Choose function(s): ");
        secondRequire.setBounds(firstRequire.getX(), firstRequire.getY() + 30, 130, 17);

        comboBoxSeries = new JComboBox(series);
        comboBoxSeries.setBounds(firstRequire.getX() + firstRequire.getWidth(), firstRequire.getY(), 100, 17);
        comboBoxSeries.setSelectedIndex(0);

        ButtonGroup buttonGroup = new ButtonGroup();
        functionCalculation = new JRadioButton("Calculation");
        functionCalculation.setBounds(secondRequire.getX() + 120, secondRequire.getY(), 100, 17);

        inputIndexField = new JTextField();
        inputIndexField.setBounds(functionCalculation.getX() + 20, functionCalculation.getY() + 20, 80, 17);
